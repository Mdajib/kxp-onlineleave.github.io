...
Class Master extends DBConnection {
	...
	function save_employee(){
		...
		$data = "";
		foreach($_POST as $k =>$v){
			if(in_array($k,array('firstname','username','type'))){
				if(!empty($data)) $data.=" , ";
				$data .= " `{$k}` = '{$v}' ";
			}
		}
	...
	function save_application(){
		...
		if($leave_days > $available){
			$resp['status'] = 'failed';
			$resp['msg'] = " Your remaining leave balance is only {$available} days !! Please apply 'Annual Leave' or 'Unpaid Leave'.";
			return json_encode($resp);
			exit;
		}
		...
		if(empty($id)){
			if (!empty($_FILES['proof_document']['name'])) {
				$proof_document_file_name = time()."-".rand(3300,88888)."-".rand(1,84558).".".pathinfo($_FILES['proof_document']['name'], PATHINFO_EXTENSION);
				if (move_uploaded_file($_FILES['proof_document']['tmp_name'],"../uploads/documents-proof/$proof_document_file_name")) {
					$data.= " , `proof_document`='$proof_document_file_name'";
